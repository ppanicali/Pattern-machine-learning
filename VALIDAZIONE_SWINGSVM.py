...
for index in range(0,len(df2_B)):
    ...
    if index >= dimension_p and index<=rows_exp-dimension_n:
        ...
        range_pattern_now = max_pattern - min_pattern
        range_pattern_perc = range_pattern_now / df2_B.at[index,'Close'] *100
        range_ok=range_pattern_perc>0.3
        ...
        third_less02 = (df2_B.at[index,'Close_n'] - df2_B.at[index,'Open_n'])< (df2_B.at[index,'Close_n']/100*0.2)
...
df_collezioneTrain=df_collezione_min[0:round(len(df_collezione_min)*0.4)]
df_collezioneTest=df_collezione_min[round(len(df_collezione_min)*0.4):len(df_collezione_min)]
